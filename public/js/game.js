...
class Player {
    constructor(gameConfigs, isCpu, isSubstitute, side, colorIndex) {
        ...
        this.powerShotWrapper = new PowerShotWrapper_1.PowerShotWrapper(gameConfigs);
    }
...
class MoveToGoalPowerShotMovementStrategy {
    ...
    apply(ball, _gameWorld, deltaMs) {
        ...
        if (distance > this.minGoalDistance) {
            ...
            ball.movementPosition.setSpeed(ball.maxSpeed * PowerShotType_1.PowerShotUtilities.getSpeedFactor(ball.ballPowerShot.getPowerShotType()), distance1 < distance2
                ? ball.movementPosition.getSpeedAngle() + Math.PI / this.ballRotateOffset * deltaMs
                : ball.movementPosition.getSpeedAngle() - Math.PI / this.ballRotateOffset * deltaMs);
        }
    }
    getDirectionDistance(ball, ballSpeedAngle) {
        ...
        const newX = ball.movementPosition.position.x +
            Math.cos(ballSpeedAngle) * dist;
        const newY = ball.movementPosition.position.y +
            Math.sin(ballSpeedAngle) * dist;
